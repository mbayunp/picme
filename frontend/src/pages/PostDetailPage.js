    ...
    const renderMedia = (postData) => {
        if (postData.media_url) {
            if (Array.isArray(postData.media_url) && postData.media_url.length > 0) {
                const imageUrls = postData.media_url.map(url => url.startsWith('http') ? url : `http://localhost:8080/assets/images/${url}`);
            ...
            if (typeof postData.media_url === 'string' && postData.media_url.trim()) {
                ...
                return (
                    <img
                        src={postData.media_url.startsWith('http') ? postData.media_url : `http://localhost:8080/assets/images/${postData.media_url}`}
                        alt={postData.title}
        ...
        if (postData.image_url) {
            const imageUrl = `http://localhost:8080/assets/images/${postData.image_url}`;
            return (
                <img
                    src={imageUrl}
                    alt={postData.title}
