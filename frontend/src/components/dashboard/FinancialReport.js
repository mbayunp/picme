...
const FinancialReport = ({ packages }) => {
    ...
    const fetchFinancialData = async () => {
        ...
        try {
            ...
            setError('Gagal memuat laporan keuangan. Pastikan backend berjalan dan endpoint sudah benar.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const aggregatedChartData = useMemo(() => {
        ...
        const sortedKeys = Object.keys(revenueData).sort((a, b) => {
            if (viewType === 'daily') {
                ...
                return moment(a.split(' ')[0], 'DD MMM').diff(moment(b.split(' ')[0], 'DD MMM'));
            } else if (viewType === 'monthly') {
    ...
    const chartOptions = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: `Laporan Keuangan ${viewType.charAt(0).toUpperCase() + viewType.slice(1)}`,
            },
        },
        scales: {
            y: {
        ...
        <div className="p-5 bg-gray-100 rounded-lg flex-grow flex flex-col">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4 mb-4">
                ...
                <div className="flex space-x-2">
                    <button
                        ...
                        className={`p-2 rounded-md ${viewType === 'daily' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
                    >
                        Harian
                    </button>
                    <button
                        onClick={() => setViewType('weekly')}
                        className={`p-2 rounded-md ${viewType === 'weekly' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
                    >
                        Mingguan
                    </button>
                    <button
                        onClick={() => setViewType('monthly')}
                        className={`p-2 rounded-md ${viewType === 'monthly' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
                    >
                        Bulanan
                    </button>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                <div className="flex justify-between items-center mb-4">
                    <p className="text-lg font-semibold">Total Pendapatan dari Booking Selesai:</p>
