...
const PackagesManager = ({ packages, fetchPackages, showModal, handleDelete }) => {
    ...
    // Filter dan Urutkan paket berdasarkan studio yang dipilih
    const filteredAndSortedPackages = useMemo(() => {
        if (!Array.isArray(packages)) return [];

        // 1. Filter berdasarkan studio
        ...
        return filtered.sort((a, b) => {
            // Urutkan berdasarkan studio dulu (jika 'All' dipilih)
            const studioComparison = (a.studio_name || '').localeCompare(b.studio_name || '');
            if (studioComparison !== 0) {
                return studioComparison;
            }
            // Jika studio sama (atau filter aktif), urutkan berdasarkan nama paket
        ...
        <div className="p-5 bg-gray-100 rounded-lg">
            ...
            <div className="mt-8">
                ...
                <div className="flex flex-col gap-4">
                    {/* Gunakan filteredAndSortedPackages */}
                    {filteredAndSortedPackages.length > 0 ? (
                        filteredAndSortedPackages.map(pkg => (
                            <div key={pkg.id} className={`bg-white p-4 rounded-lg shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-opacity duration-300 ${!(pkg.is_active === 1 || pkg.is_active === true) ? 'opacity-50 bg-gray-50' : ''}`} >
                                {/* Gambar & Info */}
                                <div className="flex items-center gap-4 flex-grow w-full md:w-auto">
                                    ...
                                    <div className="flex-grow">
                                        <div className="flex items-center gap-2 mb-1 flex-wrap"> <h4 className="font-semibold text-base md:text-lg">{pkg.nama_paket}</h4> {/* Badge Status */} {(pkg.is_active === 1 || pkg.is_active === true) ? (<span className="text-xs font-medium px-2 py-0.5 rounded bg-green-100 text-green-800">Aktif</span>) : (<span className="text-xs font-medium px-2 py-0.5 rounded bg-red-100 text-red-800">Nonaktif</span>)}</div>
